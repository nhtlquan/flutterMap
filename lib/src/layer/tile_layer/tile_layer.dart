...
class _TileLayerState extends State<TileLayer> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        final tilesToRender = _tileZoom == null ? _tileManager.all() : _tileManager.sortedByDistanceToZoomAscending(options.maxZoom, _tileZoom!);
        ...
        final tileWidgets = <Widget>[
          for (var tile in tilesToRender)
            TileWidget(
              tile: tile,
              isMuti: tilesToRender.length == 4,
              size: getTileSize(),
              tileTransformation: zoomToTransformation[tile.coords.z] ??
                  (zoomToTransformation[tile.coords.z] = _transformationCalculator.transformationFor(
                    tile.coords.z,
                    map,
                  )),
        ...
        tilesContainer = Stack(
          children: tileWidgets,
        );

        Widget? tilesLayer;
        if (options.centerDraw != null) {
          var pos = map.project(options.centerDraw!);
          pos = pos.multiplyBy(map.getZoomScale(map.zoom, map.zoom)) - map.getPixelOrigin();
          var offset = Offset(pos.x.toDouble(), pos.y.toDouble());
          final r = const Distance().offset(options.centerDraw!, options.radiusMeter!, 180);
          var rpos = map.project(r);
          rpos = rpos.multiplyBy(map.getZoomScale(map.zoom, map.zoom)) - map.getPixelOrigin();

          var realRadius = rpos.y - pos.y;
          var circleClipper = CircleClipper(offset, realRadius.toDouble());
          tilesLayer = ClipPath(
            clipper: circleClipper,
            child: tilesContainer,
          );
        } else {
          tilesLayer = options.tilesContainerBuilder == null
              ? tilesContainer
        ...
        return Stack(
          children: [
            ...
            // if (options.rotateCompass != null) compassWidget(),
          ],
        );
      },
    );
  ...
  Level? _updateLevels() {
    ...
    final toRemove = _transformationCalculator.whereLevel((levelZoom) => levelZoom != zoom && !_tileManager.anyWithZoomLevel(levelZoom));
  ...
  void _resetGrid() {
    ...
    _wrapX = crs.wrapLng;
    if (_wrapX != null) {
      final first = (map.project(LatLng(0, crs.wrapLng!.item1), tileZoom).x / tileSize.x).floorToDouble();
      final second = (map.project(LatLng(0, crs.wrapLng!.item2), tileZoom).x / tileSize.y).ceilToDouble();
      _wrapX = Tuple2(first, second);
    }

    _wrapY = crs.wrapLat;
    if (_wrapY != null) {
      final first = (map.project(LatLng(crs.wrapLat!.item1, 0), tileZoom).y / tileSize.x).floorToDouble();
      final second = (map.project(LatLng(crs.wrapLat!.item2, 0), tileZoom).y / tileSize.y).ceilToDouble();
  ...
  void _update(LatLng? center) {
    ...
    //   pixelBounds = _latLngBoundsToPixelBounds(options.tileBounds!, _tileZoom!);
    // }
    ...
    for (var j = tileRange.min.y; j <= tileRange.max.y; j++) {
      for (var i = tileRange.min.x; i <= tileRange.max.x; i++) {
        ...
        if (!_tileManager.markTileWithCoordsAsCurrent(coords)) {
          if (options.tileBounds != null) print('iFinal: ' + i.toString() + ', jFinal: ' + j.toString());
          queue.add(coords);
        }
      }
    }
    ...
    for (final coords in queue) {
      if (options.tileBounds != null) print(options.tileProvider.getTileUrl(coords, options));
      final newTile = Tile(
        coords: coords,
        tilePos: _getTilePos(coords),
        current: true,
        imageProvider: options.tileProvider.getImage(coords.wrap(_wrapX, _wrapY), options),
  ...
  Bounds _latLngBoundsToPixelBounds(LatLngBounds bounds, double thisZoom) {
    final swPixel = map.project(bounds.southWest!, thisZoom).ceil();
    final nePixel = map.project(bounds.northEast!, thisZoom).floor();
  ...
  void _tileReady(Coords<double> coords, dynamic error, Tile? tile) {
    ...
    final fadeInStart = tile.loaded == null ? options.tileFadeInStart : options.tileFadeInStartWhenOverride;
  ...
  CustomPoint _getTilePos(Coords coords) {
    final level = _transformationCalculator.getOrCreateLevel(coords.z as double, map);
...
class CircleClipper extends CustomClipper<ui.Path> {
  final double radius;
  ...
  ui.Path getClip(Size size) {
    return ui.Path()..addOval(Rect.fromCircle(center: positionOffset, radius: radius));
  }
