  ...
  Positioned _positionedForOverlay(OverlayImage overlayImage) {
    ...
    final bottomRightPixel =
        map.project(overlayImage.bounds.southEast).multiplyBy(zoomScale) -
            pixelOrigin;
    final posImage =
        map.project(overlayImage.center).multiplyBy(zoomScale) -
            pixelOrigin;
    double size;
    var width = (bottomRightPixel.x - upperLeftPixel.x).toDouble();
    var height = (bottomRightPixel.y - upperLeftPixel.y).toDouble();
    if(width>height)
      size = width;
    else
      size = height;
    var padding =  (map.zoom - 10) * 15;
    print(padding);
    size =size +padding ;
    ...
    return Positioned(
      ...
      left: posImage.x.toDouble() - size/2,
      top: posImage.y.toDouble()- size/2,
      width:size  ,
      height: size,
      child: Image(
